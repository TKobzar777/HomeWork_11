...
class Record:
    ...
    def __str__(self):
        return f'Name: {self.name} -- list phones: {" ,".join([str(pn) for pn in self.list_phones]) if self.list_phones else "no phone"} -- Birthday: {self.birthday} -- {self.days_to_birthday()} days until birthday'
